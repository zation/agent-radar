  ...
  return <section className="mx-auto max-w-[1480px] px-5 py-6 md:px-8 md:py-8">
    <section className="rounded-xl bg-[#12382f] p-5 text-[#eff9f5] shadow-lg md:p-6">
      {collapsed ? <div className="grid grid-cols-[auto_1fr_auto] items-center gap-4"><CheckCircle2 className="size-6 text-emerald-300" /><span className="min-w-0"><strong className="block truncate text-base">{query}</strong><small className="mt-1 block text-sm text-emerald-100/70">{ranked.length} candidates ranked</small></span><Button className="text-emerald-50 hover:bg-white/10" onClick={() => setManualExpanded(true)} variant="ghost">Edit<ChevronDown /></Button></div> : <>
        <div><span className="font-mono text-xs font-semibold tracking-[.13em] text-emerald-300 uppercase">Ask Agent Radar</span><h1 className="mt-2 text-2xl font-semibold tracking-tight md:text-3xl">What are you trying to build?</h1><p className="mt-1 text-base text-emerald-100/70">Describe the task. Radar ranks verified tools and preserves safety boundaries.</p></div>
        <Textarea className="recommend-task-input mt-5 min-h-20 border-emerald-600 bg-white/10 text-base text-white placeholder:text-emerald-100/55 focus-visible:border-emerald-300 focus-visible:ring-emerald-300/25" onChange={(event) => setQuery(event.target.value)} placeholder="Describe a development task" value={query} />
        {uiState.inlineMessage ? <p className={`mt-3 flex items-start gap-2 border-t border-emerald-700/70 pt-3 text-sm ${uiState.kind === "error" ? "text-red-300" : uiState.kind === "ask_human" ? "text-amber-300" : "text-emerald-100/70"}`} role={uiState.kind === "error" ? "alert" : "status"}>{uiState.kind === "error" || uiState.kind === "ask_human" ? <TriangleAlert className="size-4 shrink-0" /> : <span>—</span>}{uiState.inlineMessage}</p> : null}
        <div className="recommend-controls mt-4 flex flex-wrap items-end gap-3"><Control label="Model"><Select onValueChange={(value) => value && setModel(value)} value={model}><SelectTrigger className="w-52 border-emerald-600 bg-white/10 text-emerald-50"><SelectValue /></SelectTrigger><SelectContent>{models.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></Control><Control label="Risk"><Select onValueChange={(value) => value && setRisk(value)} value={risk}><SelectTrigger className="w-32 border-emerald-600 bg-white/10 text-emerald-50"><SelectValue /></SelectTrigger><SelectContent>{(["low", "medium", "high"] as const).map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></Control><Control label="API key" icon={<KeyRound className="size-4" />}><Input className="w-64 border-emerald-600 bg-white/10 text-emerald-50 placeholder:text-emerald-100/50" onChange={(event) => setApiKey(event.target.value)} placeholder="Used for this request only" type="password" value={apiKey} /></Control>{result ? <Button className="text-emerald-100 hover:bg-white/10" onClick={clearTask} variant="ghost"><X />Clear task</Button> : null}<Button className="recommend-run ml-auto h-10 min-w-28 bg-emerald-300 text-base font-semibold text-emerald-950 hover:bg-emerald-200" disabled={submitting || !query.trim()} onClick={() => void runRecommendation()}>{submitting ? <LoaderCircle className="animate-spin" /> : null}{submitting ? "Running" : "Run"}</Button></div>
      </>}
    </section>
    ...
    {selected ? <div className={`mt-5 grid gap-5 lg:grid-cols-[340px_minmax(0,1fr)] ${mobile.detailOpen ? "[&_.tool-index]:hidden lg:[&_.tool-index]:block" : "[&_.tool-detail-mobile]:hidden"}`}><aside className="tool-index grid min-w-0 content-start gap-1 rounded-xl border border-border bg-card p-2 shadow-sm lg:max-h-[max(60vh,640px)] lg:overflow-y-auto">{rows.map((row) => <ToolRow active={row.tool.card.id === selected.tool.card.id} key={row.tool.card.id} onSelect={(trigger) => { setSelectedId(row.tool.card.id); mobile.openDetail(row.tool.card.id, trigger); }} rank={row.rank} recommendationLevel={row.recommendationLevel} score={row.score} tool={row.tool} />)}</aside><div className={mobile.detailOpen ? "tool-detail-mobile block" : "hidden lg:block"}><ToolDetail mobileOpen={mobile.detailOpen} onMobileBack={mobile.closeDetail} taskReason={result ? getTaskReason(selected.tool.card.id, result) : undefined} tool={selected.tool} /></div></div> : <p className="mt-5 rounded-xl border border-dashed border-border p-10 text-center text-muted-foreground">No tools match the current filters.</p>}
  </section>;
}
