...
export function ToolsWorkspace({ tools }: { tools: ToolViewModel[] }) {
  ...
  return <section className="tools-page page-frame">
    ...
    {selected ? <div className="tools-workspace"><aside className="tool-index">{rows.map((row) => <button aria-current={row.tool.card.id === selected.tool.card.id ? "true" : undefined} className={row.tool.card.id === selected.tool.card.id ? "is-selected" : ""} key={row.tool.card.id} onClick={() => setSelectedId(row.tool.card.id)} type="button"><i className={`tool-type-dot is-${row.tool.card.type}`} /><span><strong>{row.tool.card.name}</strong><small>{row.tool.card.type} · {row.recommendationLevel}</small></span><em>{row.rank ? String(row.rank).padStart(2, "0") : row.score}</em></button>)}</aside><ToolDetail taskReason={result ? getTaskReason(selected.tool.card.id, result) : undefined} tool={selected.tool} /></div> : <p className="empty-state">No tools match the current filters.</p>}
  </section>;
}
