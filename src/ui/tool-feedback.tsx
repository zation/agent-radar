...
export function ToolFeedback({ toolId }: { toolId: string }) {
  ...
  return <section aria-label="Tool feedback" className="mt-5 flex flex-wrap items-center gap-3 rounded-lg border border-border bg-muted/45 p-4">
    ...
    <Dialog onOpenChange={setDialogOpen} open={dialogOpen}><DialogContent><DialogHeader><DialogTitle>Add details on GitHub?</DialogTitle><DialogDescription>Your vote is saved. You can optionally explain what worked or what should improve in a structured GitHub issue.</DialogDescription></DialogHeader><DialogFooter showCloseButton><Button render={<a href={issueUrl({ toolId, vote: lastVote, release: feedback.version.release_id, dataVersion: feedback.version.data_version })} rel="noreferrer" target="_blank" />}>Add details</Button></DialogFooter></DialogContent></Dialog>
  </section>;
}
