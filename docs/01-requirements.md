...
系统应维护可采集来源注册表，用于记录官方 registry、GitHub 组织、文档站、包管理源、awesome list 和社区目录。

验收标准：

- 每个来源包含名称、URL、来源类型、工具类型覆盖、采集方式、建议频率、可信度、速率限制和使用限制。
- 新增来源必须说明用途、预期字段、合法性边界和失败处理。
- 不采集需要绕过登录、付费墙、验证码或服务条款限制的数据。
...
MVP 应提供轻量 MCP 查询能力或与 MCP 等价的结构化接口，供 coding agent 查询工具卡片和推荐结果。

验收标准：

- 至少支持 `search_tools`、`get_tool_card`、`recommend_tools`、`explain_rating`。
- 接口只读取公开数据和本地索引，不执行第三方工具安装或授权。
- 错误返回包含可读原因和可恢复建议。
...
MVP 应优先使用静态文件、GitHub Actions、SQLite/DuckDB 和低成本托管。

验收标准：在无付费基础设施的情况下可以生成数据、运行评测并发布静态索引。
...
- 少量高质量公开来源注册。
- 可人工复核的采集与标准化流程。
- 静态 JSON/JSONL 数据集和基础索引。
- 基础评分规则和解释模板。
- 面向开发任务的推荐输出。
- 轻量 MCP 查询接口或等价 JSON API。
- Golden queries 和推荐质量评测。

MVP 可以使用人工补全和半自动采集，不要求全自动覆盖生态。
