...
- `reports/*`：eval report。
- `artifact-manifest.json`：记录 git sha、data version、rules version、eval provider/model、通过数、eval failure categories、source registry diff summary、Tool Card URL validation summary、crawl audit summary、ingestion approval summary、release admission summary、promotion candidates summary、构建时间和关键文件 checksum。
...
- `artifact-manifest.json` 的摘要，包括 git sha、data version、eval 通过数、source registry diff summary、Tool Card URL validation summary、crawl audit summary、ingestion approval summary、release admission summary、promotion candidates summary 和 checksum 数量。
