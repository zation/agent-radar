...
`all-v0.7.1` is the current verified baseline. Release All run `29344911914` and production deployment `5443347512` bind commit `595c3ee9fd49375aa3405cfdb5ebd3903d37d996`, reviewed bundle `agent-radar-all-29344911914`, production evidence, and the Worker endpoint. The deployed catalog contains 76 Tool Cards, including 23 dynamically discovered Skills. Provider evaluation passed 24/24, critical safety passed 4/4, and MCP smoke passed 7/7. The reviewed usage evidence records 24/24 reported requests, no unavailable usage or retries, and 965,948 total tokens. The active/latest official Registry record remains `io.github.zation/agent-radar@0.6.4` and points to the same production remote.
