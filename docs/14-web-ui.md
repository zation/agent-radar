...
Review 页面当前保持只读。它帮助维护者把 preview/Actions 中的来源审核请求结构化扫描出来，但不会在浏览器端写入 review record，也不会自动确认来源、启用 crawler 或发布 Tool Card。
...
- 通过右上角状态弹层查看 eval 状态。
- 比较少量候选。
- 查看 Source Registry review confirmation requests。

MVP Web UI 不允许：

- 安装第三方工具。
- 存储、记录或把用户 API key、secret、token、邮箱内容或私有代码写入 artifacts、日志或页面状态以外的持久位置。
- 修改 Tool Card、评分规则、review records 或来源数据。
