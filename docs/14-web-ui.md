...
当前模型下拉由构建期共享的 `src/recommendation/provider-registry.ts` 经 `src/ui/provider-options.ts` 提供，不在浏览器运行时读取 `data/provider_registry.json`。更完整的 Provider runtime 配置 UI、浏览器运行时 registry 加载，以及 direct-to-provider 与 proxy 模式选择均延期到 v0.3 或以后，不属于 v0.2；v0.2 固定使用浏览器到 Worker、Worker 到 provider 的后端代理路径。
...
- 在浏览器运行时修改 Provider 配置、读取 Provider registry 或直连第三方 provider；这些能力延期到 v0.3 或以后。
...
- Provider runtime 配置 UI、浏览器读取 `provider_registry.json` 和 direct-to-provider 模式属于 v0.3 或以后；若要提前，必须先更新产品、架构、安全和部署边界。
