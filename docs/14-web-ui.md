...
- 展示 pending Source Registry production gate review requests。
- 标出需要 production gate 关注的高影响来源字段。
- 展示 review reason 和 suggested action。

Review 页面只读展示 reviewed bundle 中的审核提示。它不生成 review record JSON，不在浏览器端写入 artifact，不自动确认来源、启用 crawler 或发布 Tool Card；人工确认发生在 GitHub production environment approval。
...
- `sourceReviewRequests`：把 Source Registry production gate review requests 提供给 Review 页面。
...
- 通过右上角状态弹层查看 eval 状态。
- 比较少量候选。
- 查看 Source Registry production gate review requests 和 suggested action。
