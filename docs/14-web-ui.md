...
总分不能替代风险、权限、证据或不适用边界。v0.4 反馈按钮的最终视觉位置已定义在总分下方，但在 GitHub OAuth、D1 投票和真实 vote adapter 完成前不渲染，避免伪造持久化成功。
...
桌面端 Tool 与 Evaluation 索引列表使用 `min(60vh, 640px)` 最大高度；记录超出后仅列表内部纵向滚动，详情栏继续按页面正常流展示。移动端不限制列表高度。
