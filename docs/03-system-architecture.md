...
Outputs: local status, search results, Tool Card lookups, rating explanations, and recommendation context bound to one verified release. Every successful local command carries the top-level source marker `agent-radar-skill`. The Skill announces its use before presenting results and ends every Agent Radar-backed answer with the exact release ID and data version returned by the client. Commit SHA remains in the structured client result for audit but is omitted from the user-facing provenance line.

Implementation boundary: `src/skill-data/release.ts` generates `agent_radar_skill_channel.v1` and immutable `agent_radar_skill_data_manifest.v1` releases from the same reviewed Tool Cards, Ratings, and Search Index used elsewhere. It revalidates inherited releases before copying them from the previous reviewed bundle. `skills/agent-radar/SKILL.md` owns trigger, invocation disclosure, response provenance, local reasoning, and action-ceiling instructions. Its dependency-free client synchronizes into a versioned cache, verifies contract compatibility, size, SHA-256, and record schemas, and atomically switches `current.json`. Local queries read one release and never call MCP or the hosted recommendation endpoint. Invocation visibility adds no telemetry or query log.
...
`all-v0.9.2` is the current verified production baseline. Release All run `29504976126`, commit `60d289bdfa6d0ac8013c4a5e7f37cf358f796eba`, and deployment `5474831332` bind the immutable reviewed bundle and `production_release_evidence.v2`; the deployed catalog contains 76 Tool Cards. Real-provider golden evaluation 24/24 and critical safety 4/4 passed. Release identity converged in one attempt with exact tag/SHA, deployed `/api/mcp` expected/actual server version was `0.9.2`, and all seven functional checks passed. Registry run `29505964111` repeated the convergence and smoke gates before publishing active/latest `io.github.zation/agent-radar@0.9.2`.
