...
Before deployment, the reviewed bundle preserves its manifest, checksums, automatic review, release admission, promotion check, and summaries. After deployment, `production-release-evidence.json` records repository, workflow run, SHA, tag, deployment ID, bundle name, manifest SHA, D1 artifact checksum, Worker and MCP endpoint, observed release identity, and smoke results. The workflow resolves the unique production deployment for the current run and validates all correlation fields. Any failure fails the release.

For v0.9 P3, deployment evidence uses `release_identity_convergence.v1`, `mcp_smoke_result.v3`, and `production_release_evidence.v2`. Tests cover immediate convergence, retry after an old edge, bounded exhaustion, malformed responses, release/SHA mismatch, exact MCP version success, and previous-version failure. Registry publication is a second evidence-bound gate: `mcp_registry_publication_evidence.v1` binds the selected successful Release All run, tag, SHA, production-evidence checksum, canonical `server.json` checksum, official Registry query URL/time, active/latest publication status/time, repository, transport, and remote endpoint. Before OIDC, the workflow rebuilds production evidence from the source run's reviewed manifest, D1 seed, convergence result, and smoke artifact, then reruns convergence and fresh version-aware smoke against the metadata endpoint. A missing record may proceed to publication; an active/latest identical record is idempotent success; a same-name/version record with a different repository or remote is an immutable conflict.
...
- Exact observed release ID, commit SHA, and tag-derived MCP server version convergence before evidence construction.
