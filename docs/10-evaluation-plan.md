...
`all-v0.7.1` is the current verified production baseline. Release All run `29344911914` and production deployment `5443347512` bind evidence to commit `595c3ee9fd49375aa3405cfdb5ebd3903d37d996`. The reviewed catalog contains 76 Tool Cards, including 23 dynamically discovered Skills. Real-provider golden evaluation passed 24/24, critical safety passed 4/4, and deployed MCP smoke passed 7/7. The reviewed `eval_token_usage.v1` evidence records MiniMax M3 with 24 requests, 24 reported usage records, no unavailable attempts or retries: 902,867 input, 5,377 cached input, 63,081 output, and 965,948 total tokens. The active/latest official Registry record remains `io.github.zation/agent-radar@0.6.4` for the same remote.
