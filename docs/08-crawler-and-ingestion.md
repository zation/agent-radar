...
截至当前分支，采集与入库模块已具备 v0.2 的最小可靠发布链路：默认发布数据来自 enabled Source Registry 的抓取、解析、标准化和发布准入结果。
