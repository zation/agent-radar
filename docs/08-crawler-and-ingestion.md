...
当前 enabled source 只有 `manual-agent-radar-seed`，用于验证 Crawl Plan、Crawl Audit、Raw Snapshot、Source Record、Tool Card draft、dedup report、approval record、approval request、field value provenance、review queue、release admission、promotion candidates 和 promotion plan 契约。完整且无 parser warnings 的 manual Source Records 会经过最小 normalizer 生成待审核 Tool Card drafts，并输出最小 duplicate report；review queue 也会标注与已发布 Tool Cards 以及同批 incoming drafts 的最小重复信号和已记录的 approval decision。repository Source Records 会额外进入 `tool_discovery_candidates.v1`，作为发现来源的待人工审核候选，不会直接生成 Tool Card drafts。preview review markdown 会列出 discovery candidate 数量和候选明细，artifact manifest summary 也会汇总 candidate 总数与 pending manual review 数量，方便维护者审核发现信号而不把社区仓库自动提升为可靠 Tool Card。approval requests 会为尚未审核的 draft 输出 approval record 模板、decision options、重复信号和 validation 背景，并在 preview markdown 中区分 `published_duplicates` 和 `draft_duplicates`，也会把可逐行处理的模板写入 `approval_record_templates.jsonl`，供维护者填入真实 decision/reviewer/reason/reviewed_at 后再进入 approval records。field value provenance 会把 draft 顶层字段映射回 `raw_fields.<field>`、Source Record id、`provenance_type` 和截断后的原始/标准化值预览；当 Override Record 改写 draft 字段且 override id 已进入 `evidence_refs` 时，也会输出 `override_record` provenance item，方便维护者检查字段和值来源。release admission 会把 `ready_for_review`、`approved` 且无已发布/同批 draft 重复信号的 draft 标为 `eligible_for_publish`。Preview review markdown 会列出字段值 provenance 样例、每个 approval request、release admission item 的 status 和 blocking reasons，帮助维护者判断 blocked draft 需要补 approval、修 validation 还是处理 duplicate；promotion candidates 会把 eligible drafts 连同 approval evidence 复制到独立候选 artifact，并额外输出可人工合并到 `seedToolCards` 的 TypeScript snippet；promotion plan 会为这些候选输出目标文件、候选 artifact 路径、seed candidate artifact 路径、推荐人工动作和发布前检查项；promotion check 会 dry-run 校验候选是否与现有 seed 重复、是否仍通过 Tool Card validator，供后续人工提升到可靠发布数据。Preview review markdown 和 artifact manifest summary 也会列出 promotion candidate、plan 与 check 明细，方便维护者审核候选而不自动发布。该步骤仍不会自动合并或发布，这些草稿仍不进入可靠发布 artifacts。`github-topic-mcp` 已登记且已有基础 `github_topic_parser` fixture，可把 GitHub topic/API payload 解析为 repository Source Records；crawler 在显式抓取 GitHub topic source 时会使用公共 GitHub Search API、无 Authorization header，并记录 rate-limit response metadata；但 source 仍保持 disabled，避免 MVP 后立即引入社区来源噪声。
...
`npm run ingest` 的终端 JSON summary 会包含 snapshots、source records、source ids、discovery candidates、approval requests、field value provenance、release admission、promotion candidates 和 promotion plan 摘要，便于本地或 CI 快速判断采集审核状态。`npm run preview:build` 会把 discovery candidates summary 同步进 artifact manifest，并把 pending manual review 候选明细写入 preview review markdown。
...
- 更多来源专属 parser；当前已有 `manual_seed_parser` 和基础 `github_topic_parser`，但 GitHub topic source 仍未启用。
...
- 完整的人工审核 UI，以及 promotion candidates 到可靠发布 artifacts 的人工提升流程；当前 preview review markdown 已展示 discovery candidates、approval request、release admission blocked reasons、promotion candidate 明细、seed candidate snippet 路径、promotion plan 人工合并提示和 promotion check dry-run 结果。
