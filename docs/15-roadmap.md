...
- 当前状态：开发中；Tasks 1–7 已在 `main` 完成，权威文档、真实数据构建、全量验证和完成收口仍待执行。
- Spec：[`v0.7 P2: Dynamic Skill Tool Card Expansion Design`](superpowers/specs/2026-07-14-v0.7-p2-dynamic-skill-expansion-design.md)（Approved）。
- Plan：[`v0.7 P2 Dynamic Skill Expansion Implementation Plan`](superpowers/plans/2026-07-14-v0.7-p2-dynamic-skill-expansion.md)（Approved；Inline Execution，直接在 `main` 开发）。
...
- 已完成实现提交：`854b6fad`（GitHub discovery 配置）、`013e6ed4`（分阶段 Skill 抓取）、`3970686a`（grouped parser 与当前快照语义）、`9eec81a0`（manifest identity）、`b4e3b5dd` 与 `33efb4ce`（统一 findings 与动态 Skill 边界）、`e8ed8b87`（Skill rating policy）、`d8b4aa61`（pipeline rating context）。
...
- P2 Spec：[`v0.7 P2: Dynamic Skill Tool Card Expansion Design`](superpowers/specs/2026-07-14-v0.7-p2-dynamic-skill-expansion-design.md)（Approved）。
- P2 Plan：[`v0.7 P2 Dynamic Skill Expansion Implementation Plan`](superpowers/plans/2026-07-14-v0.7-p2-dynamic-skill-expansion.md)（Approved；Inline Execution）。
- 当前优先级：完成权威文档、真实 top-2 数据构建与全量 release-quality 验证；生产发布仍需单独人工批准。
