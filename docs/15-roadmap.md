...
实际情况：MVP、v0.2、v0.3、v0.4 和 v0.5 均已完成。`all-v0.5.1` 是当前已验证生产基线；Release All run `29248755693`、production deployment `5424142098`、24/24 real-provider golden eval、4/4 critical safety 和 4/4 MCP smoke 均已通过。v0.6 的代码、metadata、发布证据和 OIDC workflow 已实现并通过本地完整门禁。不可变 tag `all-v0.6.0` 的 run `29265966080` 因 DeepSeek HTTP 402 失败；`all-v0.6.1` 的 run `29267532188` 揭示 MiniMax 中国区 base URL 未注入；`all-v0.6.2` 的 run `29268110255` 则因默认 endpoint 单元测试未隔离已注入的 CI 环境变量而失败。三次均在生产审批/部署前被门禁拒绝，Registry 均未发布，tag 均不移动、不复用。当前发布目标推进为 `all-v0.6.3` / Registry `0.6.3`，仍不得提前标记为已发布。
...
- 独立 `Publish MCP Registry` workflow 已实现 successful Release All run 绑定、evidence-SHA checkout、生产 version/fresh smoke 复验、固定 publisher checksum、GitHub OIDC、幂等/冲突判断、官方 API polling 与 `mcp_registry_publication_evidence.v1` 上传；`0.6.0`–`0.6.2` 均未发布，当前待发布 `0.6.3`。
...
- 当前状态：SDK handler、shared contracts/service、secret header、七项 smoke、remote-only metadata、Registry evidence 与 GitHub OIDC workflow 均已实现；`all-v0.6.0`–`all-v0.6.2` 均在部署前 fail-closed 且未发布 Registry，当前待执行 `all-v0.6.3` production 与 Registry publication。
...
- 完成门禁：`all-v0.6.3` production evidence、24/24 real-provider eval、4/4 critical safety、部署后 MCP smoke、官方 Registry publication 和 Registry API 查询 evidence 全部通过。
