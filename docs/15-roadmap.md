...
实际情况：MVP、v0.2、v0.3、v0.4 和 v0.5 均已完成。`all-v0.5.1` 是当前已验证生产基线；Release All run `29248755693`、production deployment `5424142098`、24/24 real-provider golden eval、4/4 critical safety 和 4/4 MCP smoke 均已通过。v0.6 的代码、metadata、发布证据和 OIDC workflow 已实现并进入本地完整门禁；`all-v0.6.0` 生产发布与官方 Registry 记录仍待执行，不得提前标记为已发布。
...
- 根目录 remote-only `server.json` 已声明 `io.github.zation/agent-radar@0.6.0`、生产 `streamable-http` endpoint、optional secret header 和无 packages 约束，并通过官方 `mcp-publisher` v1.8.0 validation。
- 独立 `Publish MCP Registry` workflow 已实现 successful Release All run 绑定、evidence-SHA checkout、生产 version/fresh smoke 复验、固定 publisher checksum、GitHub OIDC、幂等/冲突判断、官方 API polling 与 `mcp_registry_publication_evidence.v1` 上传；尚未实际发布 `0.6.0`。
...
- 当前状态：SDK handler、shared contracts/service、secret header、七项 smoke、remote-only metadata、Registry evidence 与 GitHub OIDC workflow 均已实现；`all-v0.6.0` production 与 Registry publication 尚未执行。
...
- 完成门禁：`all-v0.6.0` production evidence、24/24 real-provider eval、4/4 critical safety、部署后 MCP smoke、官方 Registry publication 和 Registry API 查询 evidence 全部通过。
