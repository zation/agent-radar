...
v0.3 已启动。当前只启动 P1 数据与可信度；P1 完成并通过阶段验收后，再启动 P2 推荐安全与评测。
...
阶段 Spec：[`v0.3 P1：数据与可信度`](superpowers/specs/2026-07-10-v0.3-p1-数据与可信度-设计.md)（已批准）。

阶段 Plan：[`v0.3 P1：数据与可信度实施计划`](superpowers/plans/2026-07-10-v0.3-p1-数据与可信度-计划.md)（草稿，等待确认）。

- Field provenance v2：多来源 lineage、转换规则版本和冲突选择依据。
- URL checker v2：超时、有限重试、状态分类、检查时间和连续失败处理。
- 跨来源字段冲突处理、alias/package key、deduper/normalizer 和发布准入增强。
- Review Summary v2：汇总 provenance、URL、duplicate、override、validation、release admission 和 promotion evidence。
- 数据质量报告。
...
4. 数据覆盖和质量报告：扩展到 50-150 张高质量 Tool Cards，并持续跟踪字段缺失、来源冲突、重复项和 URL 状态。
