...
实际情况：MVP、v0.2、v0.3、v0.4、v0.5 和 v0.6 均已完成，v0.7 P1 已完成本地实现，正在等待真实 provider Release All 生产证据。`all-v0.6.4` 是当前已验证生产基线；Release All run `29307115828`、commit `f7902af30e2d566c0a7900a8e03ed00e9067a856`、production deployment `5435538293`、24/24 real-provider golden eval、4/4 critical safety 和 7/7 MCP smoke 均已通过。Registry workflow run `29307691850` 已上传可下载 publication evidence，官方 `io.github.zation/agent-radar@0.6.4` 记录为 active/latest。

v0.7 P1 已实现 provider usage 归一化、per-attempt retry 统计、`eval_token_usage.v1`、reviewed-bundle 校验和 GitHub Summary；取得至少一次真实 provider usage evidence 后，再关闭 P1 并设计 P2 Tool Card 数据扩展。独立 Data Build、定时扩量、prompt caching/batching 和 Top-K 预检索不属于当前批准范围。
...
- 当前状态：本地实现已完成，等待真实 provider Release All evidence 后完成生产验收并冻结 Spec/Plan。
...
- 本地功能提交：`5e9cfdc9`、`08bed60a`、`b053b226`、`09cb9168`。本地门禁为 `npm test` 398/398、公开语言检查、lint、stylelint、Pages build 和 diff check 全部通过；真实 provider evidence 仍待 Release All。

### P2：Tool Card 数据扩展

- 当前状态：待 P1 取得真实 provider usage evidence 后单独设计。
...
- P1 Spec：[`v0.7 P1: Golden Query Token Usage Observability Design`](superpowers/specs/2026-07-14-v0.7-p1-golden-query-token-usage-design.md)（Approved；Implementation commits: None）。
- P1 Plan：[`v0.7 P1 Golden Query Token Usage Implementation Plan`](superpowers/plans/2026-07-14-v0.7-p1-golden-query-token-usage.md)（Approved；Implementation commits: None）。
- 当前优先级：运行一次真实 provider Release All，核验 token usage availability、artifact checksum 和 GitHub Summary；证据通过后冻结 P1，并设计 P2 Tool Card 扩展批次。
