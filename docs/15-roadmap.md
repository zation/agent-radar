...
- 独立 `Publish MCP Registry` workflow 已实现 successful Release All run 绑定、evidence-SHA checkout、生产 version/fresh smoke 复验、固定 publisher checksum、GitHub OIDC、幂等/冲突判断、官方 API polling 与 `mcp_registry_publication_evidence.v1` 上传；首次针对成功 `all-v0.6.3` 的 Registry run `29269587581` 因 checkout 前 `gh run download` 未显式指定仓库而失败，生产不受影响，修复后使用同一成功 Release All run ID 重放。
