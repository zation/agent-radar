...
实际情况：MVP、v0.2、v0.3、v0.4、v0.5、v0.6、v0.7、v0.8 和 v0.9 均已完成。`all-v0.9.2` 是当前已验证生产基线；Release All run `29504976126`、commit `60d289bdfa6d0ac8013c4a5e7f37cf358f796eba`、production deployment `5474831332`、76 张 Tool Card、24/24 real-provider golden eval 和 4/4 critical safety 均已通过。P3 `release_identity_convergence.v1` 在 attempt 1 观察到 expected/actual release `all-v0.9.2` 与 commit SHA 完全一致；`mcp_smoke_result.v3` 观察到 expected/actual `serverInfo.version=0.9.2`，七项功能检查全部通过。Registry run `29505964111` 在 OIDC publication 前复用同一收敛门禁与 fresh exact-version smoke，官方 `io.github.zation/agent-radar@0.9.2` 已为 active/latest。已安装 Skill 客户端 `0.9.1` 成功原子同步 `all-v0.9.2` dataset，返回 `source=agent-radar-skill`、`status=ready`。
...
- 统一的 Agent Radar SVG 品牌标志已接入 README、Web 顶部导航和 favicon，三个公开入口复用紧裁画布的 `public/logo.svg`。
...
在扩大 Tool Card 数量前建立真实 provider token usage 可观测性，再用实际成本和推荐质量证据约束数据扩展批次。

### P1：Golden Query Token Usage

- 当前状态：已完成生产验收，Spec/Plan 已冻结；其成本基线已用于 P2 Tool Card 数据扩展。
- Spec：[`v0.7 P1: Golden Query Token Usage Observability Design`](superpowers/specs/2026-07-14-v0.7-p1-golden-query-token-usage-design.md)。
- Plan：[`v0.7 P1 Golden Query Token Usage Implementation Plan`](superpowers/plans/2026-07-14-v0.7-p1-golden-query-token-usage.md)。
- 为每个 Golden Query provider request attempt 记录 reported/unavailable usage、重试和稳定 case identity。
- 生成 `eval_token_usage.v1`、reviewed-bundle checksum 和 GitHub Actions Summary。
- 缺失或异常 provider usage 不阻断发布；usage evidence 自身损坏仍阻断 finalization。
- 不修改公开 Recommendation Result、HTTP、MCP 或 Web contract。
- 实现提交：`5e9cfdc9`、`08bed60a`、`b053b226`、`09cb9168`、`f4f53290`。本地门禁为 `npm test` 398/398、公开语言检查、lint、stylelint、Pages build 和 diff check 全部通过。
- 生产证据：`all-v0.7.0` 的 Release All run `29326321908` 和 production deployment `5439377930` 绑定提交 `f4f53290367d0e5bfda6347f2f372f7cca061c6e`、reviewed bundle、24/24 Golden Queries、4/4 critical safety、7/7 MCP smoke 与 `eval_token_usage.v1`。MiniMax M3 共执行 24 次请求，reported 24、unavailable 0、retry 0；input 581,819、cached input 3,272、output 57,976、total 639,795，平均 26,658.125。

### P2：Tool Card 数据扩展

- 当前状态：已完成实现、release-quality 验证和生产验收，Spec/Plan 已冻结；v0.7 已正式收口。
- Spec：[`v0.7 P2: Dynamic Skill Tool Card Expansion Design`](superpowers/specs/2026-07-14-v0.7-p2-dynamic-skill-expansion-design.md)（Completed）。
- Plan：[`v0.7 P2 Dynamic Skill Expansion Implementation Plan`](superpowers/plans/2026-07-14-v0.7-p2-dynamic-skill-expansion.md)（Completed；Inline Execution，直接在 `main` 开发）。
- 首批 Source 使用 `topic:agent-skills` 按 stars 降序发现 top 2 repositories，并把根目录 `skills/**/SKILL.md` 展开为一张 Skill 一张 Tool Card；不把 repository collection 本身作为 Skill 卡。
- 该 Source 以当前 build 成功抓取的数据为权威快照：不合并或回退上一轮 Skill records，搜索、repository tree 或 manifest 失败时只保留本轮其他成功 records，排名变化可以移除旧卡。
- 保持 `tool_card.v1`、`tool_card_auto_review.v1` 和 `rating_result.v2` 公共 envelope；Auto Review 统一内部 finding 接口，Rating 统一 core/policy 接口并先实现 Skill 类型规则。
- 以开发场景覆盖缺口为主要选择依据，扩展批次必须继续通过 provenance、conflict、duplicate、URL、promotion、24/24 Golden Query 和 critical safety 门禁。
- 批次数量由 P1 的实际 input/output/total token、延迟和推荐回归证据决定，不预先引入 prompt 优化架构。
- 实现提交：`854b6fad`（GitHub discovery 配置）、`013e6ed4`（分阶段 Skill 抓取）、`3970686a`（grouped parser 与当前快照语义）、`9eec81a0`（manifest identity）、`b4e3b5dd` 与 `33efb4ce`（统一 findings 与动态 Skill 边界）、`e8ed8b87`（Skill rating policy）、`d8b4aa61`（pipeline rating context）、`16c654a2`（权威文档）和 `0fbdcc6c`（Skill runtime 权限映射）。
- 本地 reviewed bundle `v0.7-p2-local` 绑定提交 `0fbdcc6c1f24b96743af9404e1eefb61e8e96309`：top-two repositories 为 `anthropics/skills` 和 `DietrichGebert/ponytail`，分别生成 17 和 6 张动态 Skill 卡；最终 76 张 Tool Card 中包含 25 Skill、30 MCP、14 Framework、6 Agent 和 1 CLI。Crawl audit 为 62/62 success；Auto Review 78 个候选中 promote 76、reject 2，promotion 76 ready、0 blocked、0 validation errors。
- 同一 bundle 的 MiniMax M3 Golden Queries 为 24/24、critical safety 4/4；24 次 request attempts 全部报告 usage，0 unavailable、0 retry。Input 902,771、cached input 3,300、output 61,361、total 964,132、平均 40,172.167；相对 P1 分别增加 320,952（55.16%）、28（0.86%）、3,385（5.84%）和 324,337（50.69%）。本轮目录从生产基线 53 张增至 76 张，新增量正好是 23 张动态 Skill，但 token 差异仍是整套 24-query build 的观测值，不是单卡成本模型。
- 生产证据：`all-v0.7.1` 的 Release All run `29344911914` 和 production deployment `5443347512` 绑定提交 `595c3ee9fd49375aa3405cfdb5ebd3903d37d996` 与 reviewed bundle `agent-radar-all-29344911914`。生产目录同为 76 张 Tool Card；Golden Queries 24/24、critical safety 4/4、MCP smoke 7/7。24 次 request attempts 全部报告 usage，0 unavailable、0 retry；input 902,867、cached input 5,377、output 63,081、total 965,948、平均 40,247.833。相对 P1 生产基线 total 增加 326,153（50.98%）。
- 本地匿名 GitHub API 额度在重复验证中耗尽后，用户批准一次仅作用于构建子进程的 `gh` transport shim；它只为 `api.github.com` 的公开请求加认证，未进入提交、日志或 artifact，构建选中的两个 repositories 均明确为 public。产品 crawler 和 CI 仍保持无 Authorization 的已批准边界。

### 不做

- 独立 Data Build 或 Data-only release。
- 定时自动扩量或自动生产发布。
- Prompt caching、multi-query batching、provider session 或 Top-K 预检索。
- Token 成本发布阈值；P1 只建立观测证据。

## v0.8

### 目标

在保持推荐候选、公开 contract、24 条 Golden Query 和确定性安全下限不变的前提下，降低 Release All 的真实 provider token 消耗；先完成无召回风险的 prompt 紧凑化，再根据真实 provider 证据决定是否设计候选预检索。

### P1：Compact Recommendation Prompt

- 当前状态：Completed；本地质量门禁、真实 provider reviewed build、production deployment、7/7 MCP smoke 和 production release evidence 均已通过。
- Spec：[`v0.8 P1: Compact Recommendation Prompt Design`](superpowers/specs/2026-07-15-v0.8-p1-compact-recommendation-prompt-design.md)。
- Plan：[`v0.8 P1 Compact Recommendation Prompt Implementation Plan`](superpowers/plans/2026-07-15-v0.8-p1-compact-recommendation-prompt.md)。
- 实现提交：`8d8c06c6`；production release commit：`c174c139`。Spec/Plan 已标记 Completed 并冻结。
- `buildRecommendationPrompt` 只移除 JSON 格式化空白，不删除字段、不改变字段顺序、不改变完整 76-card catalog、不引入 Top-K，也不修改 provider request、Recommendation Result、HTTP、MCP 或 Web contract。
- 本地同一 76-card prompt 的序列化体积从 163,104 bytes 降至 119,952 bytes，减少 43,152 bytes（26.46%）；该结果只是 byte 证据，真实 token delta 以 provider-reported usage 为准。
- 验收必须保持 24/24 Golden Queries、4/4 critical safety、有效 `eval_token_usage.v1`，并与 `all-v0.7.1` 的 902,867 input / 63,081 output / 965,948 total tokens 比较。
- 首次真实尝试：`all-v0.8.0`、Release All run `29383566104`、commit `c174c13913d82cf14c67f4cda060d38a2b4d5781`。结果为 23/24；唯一失败是 major case `gq-secrets-access-approval`，初始请求和两次瞬时错误重试均达到 120 秒 provider timeout。其余 23 条（包括 4/4 critical safety）通过。Release gate 在 reviewed-bundle 上传前终止，production job 自动跳过，因此 runner-local token artifact 未被保留，本次不能形成完整 token delta。
- 同一 run 的 attempt 2 重跑成功：24/24 Golden Queries、4/4 critical safety；24 次请求均完成并报告 usage，0 unavailable、0 retry。Reviewed artifact 记录 input 638,532、cached input 3,484、output 61,845、total 700,377、平均 29,182.375。相对 `all-v0.7.1`，input 减少 264,335（29.28%），output 减少 1,236（1.96%），total 减少 265,571（27.49%）。Production deployment `5459363215` 恢复同一 reviewed bundle，7/7 MCP smoke 与 production release evidence 通过，v0.8 P1 完成。

### Cache 探索记录

v0.8 P1 前进行了两组各两次、严格顺序执行的 MiniMax M3 诊断请求，均未修改或持久化 prompt、response 或凭证：

- 单一 user message 中把稳定 catalog 放在动态 query 前：两次 input 分别为 26,604 和 26,598，cached input 均为 128，命中率约 0.48%。
- 把相同的 119,756-byte catalog context 放进独立 system message，user message 只保留 171-197 bytes query：两次 input 分别为 26,568 和 26,562，cached input 为 114 和 128，命中率约 0.43% 和 0.48%。
- 四次请求均返回 HTTP 200 和有效 JSON，但没有复用约 26.5k-token catalog。因此当前 MiniMax M3 route 的自动 cache 不能作为 build 降耗前提；P1 不继续依赖 provider cache。

### P2 决定：不做 Recommendation Retrieval

- P1 已在不缩减候选集的前提下把 total token 降低 27.49%，并保持 24/24 Golden Queries 与 4/4 critical safety。
- Top-K/shortlist 会引入候选召回、类型/标签覆盖、高风险候选和低权限替代项遗漏风险，因此 v0.8 不启动 P2，不创建 Spec/Plan。
- 未来只有在新的成本或目录规模证据证明紧凑 prompt 不足时，才将 retrieval 作为新的 Roadmap increment 重新评估；不得把本次取消视为已批准的后续范围。

### 暂不做

- 同一 provider session 中串行执行 24 轮对话。
- 把自动 prompt cache 当作发布成本保证。
- Multi-query batching、减少 Golden Query 数量或放宽 critical safety。
- 在形成真实 compact-prompt baseline 前设置 token release threshold。
...
- v0.3：推荐安全解释、确定性安全下限和 critical safety 发布门禁；Eval Diff 移入 Backlog。
...
当前没有已批准的新阶段 Spec 或 Plan。以下 v0.2 至 v0.6 条目是完成记录，不再作为待执行清单；尚未排期的候选工作统一保留在本节末尾的 Backlog，进入开发前需先建立新的小范围 Spec/Plan。
...
- Web UI 视觉与交互 Spec：[`v0.4 P1：Web UI 视觉与交互重构`](superpowers/specs/2026-07-11-v0.4-p1-web-ui-视觉与交互重构-设计.md)（已完成并冻结）。
...
- P1 UI 重构与反馈写链路已由 `all-v0.4.1` 完成生产验收。
...
- P2 Spec：[`v0.4 P2：反馈处理与评级接入`](superpowers/specs/2026-07-12-v0.4-p2-feedback-processing-rating-design.md)（已完成并冻结）。
- P2 Plan：[`v0.4 P2：反馈处理与评级接入实施计划`](superpowers/plans/2026-07-12-v0.4-p2-feedback-processing-rating.md)（已完成并冻结）。
- P2 已由 `all-v0.4.4` 完成生产验收；三态 Issue 处理、投票快照和 `feedback_rules.v0.1` 已接入 `Release All` 的 reviewed bundle 构建。
...
- Spec：[`v0.6：官方 MCP TypeScript SDK 与 Registry 发布设计`](superpowers/specs/2026-07-13-v0.6-official-mcp-sdk-registry-design.md)（Completed；已冻结）。
- Plan：[`v0.6 Official MCP SDK and Registry Publication Implementation Plan`](superpowers/plans/2026-07-13-v0.6-official-mcp-sdk-registry.md)（Completed；Tasks 1–10 已完成并冻结）。
- 当前状态：`all-v0.6.4` 已部署至 production；SDK handler、shared contracts/service、secret header、公开参数 description 和七项 smoke 均已通过。官方 Registry 的 `io.github.zation/agent-radar@0.6.4` 为 active/latest，publication evidence 已上传。
...
- 完成证据：Release All run `29307115828`、deployment `5435538293`、Registry run `29307691850`；production artifact `agent-radar-mcp-smoke-29307115828` 与 Registry artifact `mcp-registry-publication-evidence-29307115828` 可下载核验。

### v0.7：评测成本可观测性与数据扩展

- P1 Spec：[`v0.7 P1: Golden Query Token Usage Observability Design`](superpowers/specs/2026-07-14-v0.7-p1-golden-query-token-usage-design.md)（Completed；已冻结）。
- P1 Plan：[`v0.7 P1 Golden Query Token Usage Implementation Plan`](superpowers/plans/2026-07-14-v0.7-p1-golden-query-token-usage.md)（Completed；已冻结）。
- P1 生产证据：`all-v0.7.0`、Release All run `29326321908`、production deployment `5439377930`、24/24 real-provider eval、4/4 critical safety、7/7 MCP smoke；24 次 request attempts 均报告 usage，total 639,795 tokens。
- P2 Spec：[`v0.7 P2: Dynamic Skill Tool Card Expansion Design`](superpowers/specs/2026-07-14-v0.7-p2-dynamic-skill-expansion-design.md)（Completed；已冻结）。
- P2 Plan：[`v0.7 P2 Dynamic Skill Expansion Implementation Plan`](superpowers/plans/2026-07-14-v0.7-p2-dynamic-skill-expansion.md)（Completed；Inline Execution；已冻结）。
- P2 本地证据：`v0.7-p2-local` 绑定 `0fbdcc6c`，76 张 Tool Card、23 张新增动态 Skill、24/24 real-provider eval、4/4 critical safety、24/24 reported usage、964,132 total tokens。
- P2 生产证据：`all-v0.7.1`、Release All run `29344911914`、production deployment `5443347512`、76 张 Tool Card、24/24 real-provider eval、4/4 critical safety、7/7 MCP smoke；24 次 request attempts 均报告 usage，total 965,948 tokens。v0.7 已完成。
- 独立 Data Build、定时扩量、prompt caching/batching 和 Top-K 预检索保留为后续候选，不进入 P1/P2 当前范围。

### v0.8：Recommendation Prompt 降耗

- P1 Spec：[`v0.8 P1: Compact Recommendation Prompt Design`](superpowers/specs/2026-07-15-v0.8-p1-compact-recommendation-prompt-design.md)（Completed；已冻结）。
- P1 Plan：[`v0.8 P1 Compact Recommendation Prompt Implementation Plan`](superpowers/plans/2026-07-15-v0.8-p1-compact-recommendation-prompt.md)（Completed；已冻结）。
- 生产证据：`all-v0.8.0`、Release All run `29383566104`、production deployment `5459363215`、24/24 real-provider eval、4/4 critical safety、7/7 MCP smoke；24 次 request attempts 均报告 usage，0 unavailable、0 retry，total 700,377 tokens，较 `all-v0.7.1` 减少 27.49%。
- P2 Recommendation Retrieval 因候选召回和推荐质量风险取消；v0.8 以完整 catalog 的 compact prompt 完成。

### v0.9：Agent Radar Skill 分发
...
- P3 状态：Completed and production-verified；实现提交 `4a16e37d`，发布提交 `60d289bd`。独立 [`v0.9 P3 Release Identity Convergence Design`](superpowers/specs/2026-07-16-v0.9-p3-release-identity-convergence-design.md) 与 [`Implementation Plan`](superpowers/plans/2026-07-16-v0.9-p3-release-identity-convergence.md) 已完成并冻结；P2 Spec/Plan 保持冻结。
...
- P3 本地证据：实现提交 `4a16e37d`；共享 `release_identity_convergence.v1` poller/CLI 已接入 Release All 与 Registry；`mcp_smoke_result.v3` 严格断言 tag-derived server version；`production_release_evidence.v2` 绑定 expected/actual release、SHA、server version、attempts 与 timestamps。全量 429/429 tests 和 public-language check 已通过。
- P3 生产证据：`all-v0.9.2` Release All run `29504976126`、deployment `5474831332` 和 Registry run `29505964111` 均成功。生产与 Registry fresh convergence 都在 attempt 1 匹配 release `all-v0.9.2` 和 commit `60d289bdfa6d0ac8013c4a5e7f37cf358f796eba`；两次 MCP smoke 均记录 expected/actual server version `0.9.2` 并通过 7/7。`production_release_evidence.v2` 与 `mcp_registry_publication_evidence.v1` 已上传，官方 Registry 状态为 active/latest。
