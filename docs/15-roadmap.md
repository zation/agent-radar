...
- 发布流水线已输出 `mcp_tools.json` 和 `mcp_examples.json`，Workers API 提供 `/api/mcp_manifest` 返回只读工具定义，并提供 `/api/mcp` 最小 MCP JSON-RPC endpoint，支持 `initialize`、`tools/list` 和只读 `tools/call`。
...
- Workers API 已提供 HTTP/JSON 路由、只读 MCP tool manifest、最小 MCP JSON-RPC endpoint 和 agent-facing JSON-RPC examples artifact；后续仍需补部署验收。
...
- 将 Tool Card 关键字段 provenance warning 扩展为 schema 级字段 provenance，并决定是否在 CI 默认启用 URL 可达性检查。
...
- 补齐 MCP 部署验收；最小 MCP JSON-RPC endpoint 和 agent-facing examples artifact 已实现。
