...
实际情况：MVP、v0.2、v0.3、v0.4 和 v0.5 均已完成。`all-v0.5.1` 是当前已验证生产基线；Release All run `29248755693`、production deployment `5424142098`、24/24 real-provider golden eval、4/4 critical safety 和 4/4 MCP smoke 均已通过。v0.6 的代码、metadata、发布证据和 OIDC workflow 已实现并通过本地完整门禁。不可变 tag `all-v0.6.0` 的 run `29265966080` 因 DeepSeek HTTP 402 失败；`all-v0.6.1` 的 run `29267532188` 揭示 Repository Variable `AGENT_RADAR_LLM_BASE_URL` 未注入 Release All，导致 MiniMax 中国区 key 被错误发送到国际 endpoint 并返回 HTTP 401。两次均在生产审批/部署前被门禁拒绝，Registry 均未发布，tag 均不移动、不复用。当前发布目标推进为 `all-v0.6.2` / Registry `0.6.2`，仍不得提前标记为已发布。
...
- 根目录 remote-only `server.json` 已推进为 `io.github.zation/agent-radar@0.6.2`，声明生产 `streamable-http` endpoint、optional secret header 和无 packages 约束；发布前继续使用官方 `mcp-publisher` v1.8.0 validation。
- 独立 `Publish MCP Registry` workflow 已实现 successful Release All run 绑定、evidence-SHA checkout、生产 version/fresh smoke 复验、固定 publisher checksum、GitHub OIDC、幂等/冲突判断、官方 API polling 与 `mcp_registry_publication_evidence.v1` 上传；`0.6.0`、`0.6.1` 均未发布，当前待发布 `0.6.2`。
...
- 当前状态：SDK handler、shared contracts/service、secret header、七项 smoke、remote-only metadata、Registry evidence 与 GitHub OIDC workflow 均已实现；`all-v0.6.0` 和 `all-v0.6.1` 分别因 provider 余额与 base URL 注入问题在部署前失败且未发布 Registry，当前待执行 `all-v0.6.2` production 与 Registry publication。
...
- 完成门禁：`all-v0.6.2` production evidence、24/24 real-provider eval、4/4 critical safety、部署后 MCP smoke、官方 Registry publication 和 Registry API 查询 evidence 全部通过。
