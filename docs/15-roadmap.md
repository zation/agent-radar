...
| v0.8 | 降低真实 provider 评测 token 消耗 | P1 紧凑 recommendation prompt、P2 候选检索待 P1 证据后单独设计 |
...
实际情况：MVP、v0.2、v0.3、v0.4、v0.5、v0.6 和 v0.7 均已完成；v0.8 P1 已批准并通过本地质量门禁和真实 provider reviewed build，等待 production environment 人工审批。`all-v0.7.1` 仍是当前已验证生产基线；Release All run `29344911914`、commit `595c3ee9fd49375aa3405cfdb5ebd3903d37d996`、production deployment `5443347512`、24/24 real-provider golden eval、4/4 critical safety 和 7/7 MCP smoke 均已通过。生产目录包含 76 张 Tool Card，其中 23 张来自首批动态 Skill 扩展；24 次请求均报告 usage，0 unavailable、0 retry，total 965,948 tokens。官方 MCP Registry 记录仍为 active/latest `io.github.zation/agent-radar@0.6.4`，指向同一生产 remote。
...
- 当前状态：Spec/Plan 已批准，本地实现、全量质量门禁和真实 provider reviewed build 已通过；production environment 等待人工审批，部署与 post-deploy smoke 尚未执行。
...
- 实现提交：`8d8c06c6`；Spec/Plan 在真实 provider 与生产验收完成前保持 Approved。
...
### P2 候选方向：Recommendation Retrieval

- Top-K/shortlist 仍是候选方向，不在 P1 中实现。
- 是否启动 P2 由 P1 的真实 token、推荐质量和运行时证据决定；进入开发前必须新建独立 Spec/Plan。
- P2 必须定义 baseline top-result recall、acceptable type/tag coverage、风险候选与低权限替代项覆盖、低置信度扩大/全量 fallback，以及 full-catalog 与 shortlist 的 24-query provider A/B 门禁。
- P2 不得让 shortlist 替代完整 catalog 上的本地确定性 safety assessment。
