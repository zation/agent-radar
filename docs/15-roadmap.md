...
实际情况：MVP、v0.2、v0.3、v0.4 和 v0.5 均已完成。`all-v0.5.1` 是当前已验证生产基线；Release All run `29248755693`、production deployment `5424142098`、24/24 real-provider golden eval、4/4 critical safety 和 4/4 MCP smoke 均已通过。v0.6 设计已批准，实施 Plan 已形成草案并等待执行方式确认；不得提前标记为已实现或已发布。
...
- Spec：[`v0.6：官方 MCP TypeScript SDK 与 Registry 发布设计`](superpowers/specs/2026-07-13-v0.6-official-mcp-sdk-registry-design.md)（已批准；Implementation commits: None）。
- Plan：[`v0.6 Official MCP SDK and Registry Publication Implementation Plan`](superpowers/plans/2026-07-13-v0.6-official-mcp-sdk-registry.md)（Draft；Implementation commits: None）。
- 当前状态：设计已批准，实施 Plan 已形成草案并等待执行方式确认；尚未实现、尚未发布 Registry。
