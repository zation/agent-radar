...
实际情况：MVP、v0.2、v0.3、v0.4、v0.5 和 v0.6 均已完成，当前进入 v0.7 P1 实施准备阶段。`all-v0.6.4` 是当前已验证生产基线；Release All run `29307115828`、commit `f7902af30e2d566c0a7900a8e03ed00e9067a856`、production deployment `5435538293`、24/24 real-provider golden eval、4/4 critical safety 和 7/7 MCP smoke 均已通过。Registry workflow run `29307691850` 已上传可下载 publication evidence，官方 `io.github.zation/agent-radar@0.6.4` 记录为 active/latest。

v0.7 P1 已批准 Golden Query token usage observability 设计，下一步编写实施 Plan；P1 完成并取得至少一次真实 provider usage evidence 后，再设计和实施 P2 Tool Card 数据扩展。独立 Data Build、定时扩量、prompt caching/batching 和 Top-K 预检索不属于当前批准范围。
...
- 当前优先级：先实施 Golden Query token usage artifact、manifest binding 和 GitHub Actions Summary，再以至少一次真实 provider release evidence 设计 P2 Tool Card 扩展批次。
