...
`all-v0.4.4` is the current verified production baseline. Release All run `29226907250` and production deployment `5419806444` bind `production-release-evidence.json` to commit `0b9fc48c`; real-provider golden evaluation passed 24/24, critical safety passed 4/4, and MCP smoke checks passed 4/4. The production D1 vote for `mcp-browser-automation` produced a `+0.2` adjustment, and all 53 Rating Results bind to the same real vote-snapshot checksum.
