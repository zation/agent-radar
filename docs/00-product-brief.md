...
- 初始分类体系，覆盖 Agent、Skill、MCP Server、CLI、Framework、Prompt/Rules。
- 少量高质量采集源和人工可复核的数据导入流程。
- 静态索引和基础搜索能力。
- 基础评分规则，优先强调适用场景、维护状态、文档质量、集成成本和证据质量。
- 面向开发任务的推荐输出，包含推荐理由、适用条件、风险和来源。
- 面向 agent 的轻量查询接口或结构化输出。
- 一组 golden queries，用于评估推荐质量。

MVP 不追求覆盖所有工具，也不追求全自动采集。早期宁可数据少但可信，也不要制造大量无法验证的工具卡片。

## 成功指标

### 推荐准确率

对 golden queries 进行人工或半自动评测，观察推荐结果是否真的适合任务。关键不是候选数量，而是前几个结果是否可用、解释是否合理。
