...
`all-v0.9.1` is the current verified production baseline. Release All run `29500799822` and production deployment `5473955458` bind the reviewed bundle to commit `8bb9cc281da0afa68a13d93705ef616445b8a743`; the reviewed catalog contains 76 Tool Cards, provider evaluation passed 24/24, critical safety passed 4/4, and functional MCP smoke passed 7/7. Official Registry `io.github.zation/agent-radar@0.9.1` is active. The release also reproduced an edge-propagation identity gap: functional smoke initially observed MCP version `0.9.0`. v0.9 P3 adds blocking observed release/SHA convergence and exact MCP server-version validation for the next release.
