...
`all-v0.9.2` is the current verified production baseline. Release All run `29504976126` and production deployment `5474831332` bind the reviewed bundle to commit `60d289bdfa6d0ac8013c4a5e7f37cf358f796eba`; the reviewed catalog contains 76 Tool Cards, provider evaluation passed 24/24, critical safety passed 4/4, and exact-version MCP smoke passed 7/7. The new blocking gate observed matching release ID and SHA on its first bounded attempt, then confirmed expected/actual MCP server version `0.9.2`. Official Registry `io.github.zation/agent-radar@0.9.2` is active/latest after repeating the same convergence and smoke gates.
